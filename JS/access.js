// Get temperature
function getTemp() {
    if(localStorage.getItem('temp' + JSON.parse(localStorage.getItem('toReserve'))) == null) localStorage.setItem('temp' + JSON.parse(localStorage.getItem('toReserve')), JSON.stringify(20));
...
function changeTemp(increment) {
    var temp = parseInt(document.getElementById('temp').innerHTML); // Get current temperature
    // If temp is under it's limits then change it
    if(temp > 0 && temp < 30) localStorage.setItem('temp' + localStorage.getItem('toReserve'), JSON.stringify(temp + parseInt(increment)));
    // If not only change based on it's limits
    else if(parseInt(increment) < 0 && temp == 30) localStorage.setItem('temp' + localStorage.getItem('toReserve'), JSON.stringify(temp + parseInt(increment)));
    else if(parseInt(increment) > 0 && temp == 0) localStorage.setItem('temp' + localStorage.getItem('toReserve'), JSON.stringify(temp + parseInt(increment))); 
...
function getResources() {
    ...
    for(var i = 0; i < userResources.length; i++) {
        if(userResources[i] > 100) { // If resource is a room
...
// End reservation
function endReservation() { window.location.href = '../HTML/payment.html'; }
